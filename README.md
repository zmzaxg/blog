# 轻社区博客系统 (Light Community Blog)

## 首次配置

系统首次部署时，会自动进入配置引导流程：

1. 使用内置测试超管账号登录（用户名: `zmzaxg`，密码: `mmaA123456`）
2. 初始化 D1 数据库（创建所有必要的数据表）
3. 配置站点信息和功能开关
4. 点击「写入数据库」将管理员账号和设置保存到 D1
5. 写入成功后，内置测试账号自动失效，以数据库中的账号为准

> ⚠️ 内置测试账号仅用于首次配置，完成初始化后将不可使用。
